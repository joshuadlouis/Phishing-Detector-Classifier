...
└── intent_model/             # Trained DistilBERT artifact (hosted on HuggingFace Hub — see below)
    ├── config.json
    ├── model.safetensors
    ├── tokenizer.json
...
> `labeled_dataset_cache.csv` (~170MB) and `intent_model/` (~267MB) are not tracked in this repository due to size constraints. The model is hosted on HuggingFace Hub (see Model Artifact section). The cache can be regenerated by running `build_classifier.py` or `colab_train.py`.
